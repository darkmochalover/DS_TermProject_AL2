...
# encoded_df.to_csv("C:/Users/user/Desktop/encoded_data.csv")
# aq_df.to_csv("C:/Users/user/Desktop/preprocessed_mbti_data.csv")
# -----------------------------------------------------------------------
'''
Scaler Part
'''
...
# 훈련 데이터 스케일링
X_train_scaled_minmax = minmax_scaler.transform(X_train)
...
'''
Radar chart for Comparison of Audio Quality with Different Scalers
'''
def plot_radar_chart(data, categories, title, scalers):
    # Number of variables
    ...
    # Plot each scaler's data
    for i, scaler_data in enumerate(data):
        values = scaler_data.tolist()
        values += values[:1]  # Complete the circle
        
        # Plot the data
        ax.plot(angles, values, linewidth=1, linestyle='solid', label=scalers[i])
...
scaled_data = [
    X_train_scaled_std.mean(axis=0)[:7],
    X_train_scaled_minmax.mean(axis=0)[:7],
    X_train_scaled_robust.mean(axis=0)[:7]
]

scalers = ["StandardScaler", "MinMaxScaler", "RobustScaler"]

# Plot radar chart
plot_radar_chart(scaled_data, categories, "Comparison of Audio Quality with Different Scalers", scalers)
